...
define('echarts/echarts', [
    ...
    Echarts.prototype = {
        _init: function () {
			...
			//mingsigui add for watermark
			componentLibrary.define('watermark', require('./component/watermark'));
			//mingsigui add for watermark
    ...
    var config = {
		...
		//mingsigui add for watermark
		 COMPONENT_TYPE_WARTERMARK: 'watermark',
		//mingsigui add for watermark
...
//mingsigui add for watermark
define('echarts/component/watermark', [
    'require',
    './base',
    'zrender/shape/Text',
    'zrender/shape/Rectangle',
    ...
    var zrArea = require('zrender/tool/area');
    var zrColor = require('zrender/tool/color');
    function WaterMark(ecTheme, messageCenter, zr, option, myChart) {
        Base.call(this, ecTheme, messageCenter, zr, option, myChart);
        this.refresh(option);
    }
    WaterMark.prototype = {
        type: ecConfig.COMPONENT_TYPE_WARTERMARK,
        _buildShape: function () {
            if (!this.watermarkOption.show) {
                return;
            }
			//获取水印的坐标和宽高信息
            this._itemGroupLocation = this._getItemGroupLocation();
        ...
        _buildItem: function () {
            var text = this.watermarkOption.text;
            var target = this.watermarkOption.target;
            var font = this.getFont(this.watermarkOption.textStyle);
			
			for(var i=0,isize=this._itemGroupLocation.length;i<isize;i++){
				var x = this._itemGroupLocation[i].x;
				var y = this._itemGroupLocation[i].y;
				var width = this._itemGroupLocation[i].width;
				var height = this._itemGroupLocation[i].height;
				...
				var textShape = {
					...
					style: {
						y: y,
						x: x,
						color: this.watermarkOption.textStyle.color,
						text: text,
						textFont: font,
						textBaseline: 'top',
						opacity:0.5
					...
					highlightStyle: {
						color: zrColor.lift(this.watermarkOption.textStyle.color, 1),
						brushType: 'fill'
					},
					hoverable: false
				};
				...
				if (this.watermarkOption.textAlign) {
					textShape.style.textAlign = subtextShape.style.textAlign = this.watermarkOption.textAlign;
				}
				this.shapeList.push(new TextShape(textShape));
			}
        ...
        _getItemGroupLocation: function () {
            ...
            var font = this.getFont(this.watermarkOption.textStyle);
            var subfont = this.getFont(this.watermarkOption.subtextStyle);
			...
			x = this.watermarkOption.x - 0;
			x = isNaN(x) ? 0 : x;
         
            var y;

			y = this.watermarkOption.y - 0;
			y = isNaN(y) ? 0 : y;
                
            
            return this._getItemsLocation(x,y,totalWidth,totalHeight);
        },
		_getItemsLocation:function(x,y,totalWidth,totalHeight){
			var canvasHeight = this.zr.getHeight();
			var canvasWidth = this.zr.getWidth();
			var c = this.watermarkOption.column;
			var r = this.watermarkOption.row;
			var c_gap = (canvasWidth-x) / c;
			var r_gap = (canvasHeight-y) / r;
			var result = [];
			for(var i=0;i<c;i++){
				for(var j=0;j<r;j++){
					result.push({
        ...
        refresh: function (newOption) {
            if (newOption) {
                this.option = newOption;
                this.option.watermark = this.reformOption(this.option.watermark);
                this.watermarkOption = this.option.watermark;
                this.watermarkOption.textStyle = this.getTextStyle(this.watermarkOption.textStyle);
            }
            this.clear();
            this._buildShape();
        }
    };
    zrUtil.inherits(WaterMark, Base);
    require('../component').define('watermark', WaterMark);
    return WaterMark;
});
//mingsigui add for watermark
