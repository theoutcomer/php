...
class UploadHandler
{
    ...
    protected $error_messages = array(
        1 => '图片超过限制大小，请控制在2M以内！',//'The uploaded file exceeds the upload_max_filesize directive in php.ini',
        ...
        'max_file_size' => '图片超过限制大小，请控制在2M以内！',//'File is too big',
    ...
    function __construct($options = null, $initialize = true, $error_messages = null) {
        ...
        if (!empty($options)) {
            ...
            $this->options = array(
                ...
                //'script_url' => '../cmc/file',
                'upload_dir' => '../htdocs/upfiles/cover/'.date("Ymd").'/',
                'upload_url' => '../htdocs/upfiles/cover/'.date("Ymd").'/',
                'user_dirs' => false,
                'mkdir_mode' => 0755,
                'param_name' => 'files',
    ...
    protected function get_download_url($file_name, $version = null) {
        ...
        if (empty($version)) {
            ...
            if(isset($file_name_array[0])&&isset($file_name_array[1])){
                $file_name = $file_name_array[0]."_".$version.".".$file_name_array[1];
            }else{
                return false;
            }
        }
    ...
    protected function create_scaled_image($file_name, $version, $options) {
        ...
        if (!empty($version)) {
            ...
            if($version == "large"){
                /*$file_name_array = explode(".",$file_name);
                if(isset($file_name_array[0])&&isset($file_name_array[1])){
                    $file_name = $file_name_array[0]."_large.".$file_name_array[1];
                }*/
                ...
                $file_name_array = explode(".",$file_name);
                if(isset($file_name_array[0])&&isset($file_name_array[1])){
                    $file_name = $file_name_array[0]."_small.".$file_name_array[1];
                }
            }
            else if($version == "ori"){
                $file_name_array = explode(".",$file_name);
                if(isset($file_name_array[0])&&isset($file_name_array[1])){
                    $file_name = $file_name_array[0]."_ori.".$file_name_array[1];
                }
            }
            else if($version == "listthumb"){
                $file_name_array = explode(".",$file_name);
                if(isset($file_name_array[0])&&isset($file_name_array[1])){
                    $file_name = $file_name_array[0]."_listthumb.".$file_name_array[1];
                }
            }
            ...
            //$version_dir = $this->get_upload_path(null, "large");
            $new_file_path = $file_path;
            /*$file_name_array = explode(".",$file_name);
            if(isset($file_name_array[0])&&isset($file_name_array[1])){
                $file_name = $file_name_array[0]."_ori.".$file_name_array[1];
    ...
    protected function trim_file_name($name, $type, $index, $content_range) {
        ...
        // Add missing file extension for known audio types:
        if (strpos($name, '.') === false &&
            preg_match('/^audio\/(mp3)/', $type, $matches)) {
            $name .= '.'.$matches[1];
        }
        $name = date("YmdHis").mt_rand(1000,9999).'.'.pathinfo($name,PATHINFO_EXTENSION);
        return $name;
    }
