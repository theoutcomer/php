...
<html><head><meta http-equiv="Content-Type" content="text/html; charset=UTF-8">
    ...
    <link type="text/css" rel="stylesheet" href="./ruanjian_files/squirrel-min.css">
    <link type="text/css" rel="stylesheet" href="./ruanjian_files/android-beta-min.css">
    <link type="text/css" rel="stylesheet" href="./ruanjian_files/pp.css">
</head><body>
<?php
...
public $max_size='22222222222222222222220480';//允许的最大文件大小
public $overwrite=true;//是否设置成覆盖模式
...
*/
public function __construct($upload_path='./upload/',$allow_type='jepg|jpg|apk',$max_size='222222222222222222222204800')
{//print_r($allow_type);die;
$this->set_upload_path($upload_path);
$this->set_allow_type($allow_type);
   // print_r($this->set_allow_type($allow_type));die;
...
//检查文件大小
if($this->upload_file[$i]['size']>$this->max_size*1024)$this->upload_file[$i]['error']=102;
//设置默认服务端文件名
...
for($i=0;$i<$this->upload_file_num;$i++)
{
        ...
        $this->check($i);
                ...
                {
                //上传文件
                        if(!@move_uploaded_file($this->upload_file[$i]['tmp_name'],iconv("UTF-8","gb2312",$this->upload_file[$i]['filename']))){
                            $upload_msg.='上传文件'.$this->upload_file[$i]['name'].' 出错:'.$this->error($this->upload_file[$i]['error']).'!<br>';
                            }else{ //echo "123";die;

                            $this->succ_upload_file[]=iconv("UTF-8","gb2312",$this->upload_file[$i]['filename']);
                            $upload_msg[]=$this->upload_file[$i]['name'];
                            $apkname_l=$this->upload_file[$i]['name'];
                                }
                }else{
                    ...
                    include "conn/conn.php";
                            $apkname_l=iconv("UTF-8","gb2312",$upload_msg[1]);
                            $apkname_img=iconv("UTF-8","gb2312",$upload_msg[0]);
                            $title=iconv("UTF-8","gb2312",$_POST['title']);
                            $sizes=$_POST['sizes'];
                            $bb=iconv("UTF-8","gb2312",$_POST['bb']);
                            $select=$_POST['select'];
                            $sql="INSERT INTO xians (`name`, `banb`,`img_url`,`type`,`sizes`,`apkname`) VALUES ('$title','$bb','$apkname_img','$select','$sizes','$apkname_l')";//print_r($sql);die;
                           //print_r($sql);die;
                            $query=mysql_query($sql);

//echo $upload_msg;
    echo "上传成功！！！";
    echo "<script>location.href='upload_show.php';</script>";
...
$upload=new File_upload('./upload/','jepg|jpg|apk');
$upload->upload();
